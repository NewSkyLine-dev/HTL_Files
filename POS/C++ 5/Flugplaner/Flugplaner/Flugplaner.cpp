...
void Flugplaner::setupUI()
{
	...
	mapWidget->setMapImage(QDir::currentPath().append("/resources/Earthmap.jpg"));
...
void Flugplaner::loadAirports()
{
	...
	for (const Airport& airport : airports) {
		if (!airport.iata.isEmpty()) {
			...
			fromCombo->addItem(displayText, airport.id);
			toCombo->addItem(displayText, airport.id);
		}
	}
...
void Flugplaner::loadAirlines()
{
	...
	QList<Airline> airlines = db.runQueryList<Airline>("SELECT * FROM Airline ORDER BY name;");
	
	for (const Airline& airline : airlines) {
		airlineCombo->addItem(airline.name, airline.id);
	}
...
void Flugplaner::updateFlightTable(const FlightPath& path)
{
	...
	for (int i = 0; i < path.segments.size(); ++i) {
		...
		QList<Airline> airlines = db.runQueryList<Airline>(
			QString("SELECT * FROM Airline WHERE id = %1;").arg(segment.airlineId)
		);
		QString airlineName = airlines.isEmpty() ? "Unbekannt" : airlines.first().name;

		// Set table cells
		flightTable->setItem(i, 0, new QTableWidgetItem(fromAirport.name));
		flightTable->setItem(i, 1, new QTableWidgetItem(toAirport.name));
		flightTable->setItem(i, 2, new QTableWidgetItem(airlineName));
		flightTable->setItem(i, 3, new QTableWidgetItem(
			QString("%1 → %2").arg(fromAirport.iata).arg(toAirport.iata)
		));
	}
